...
1. 从最新 `develop`（或项目当前指定的集成分支）创建 `feature/<短名>` 或 `fix/<短名>` 分支。
2. 一个提交只解决一个主题，采用 Conventional Commits 前缀；生成物和临时文件放在 `D:\Develop\AI\codex-work\riscv`，不提交到仓库。
...
5. 合并请求写明：目的、影响范围、运行的精确命令、结果、已知限制以及需要审查者关注的波形/提交轨迹。
