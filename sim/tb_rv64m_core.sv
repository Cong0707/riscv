...
module tb_rv64m_core;
  ...
  localparam string HEX_FILE = "sim/generated/rv64m_smoke.hex";
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (rst_ni && d_valid_o && d_ready_i) begin
      if (d_write_o) begin
        ...
        if (d_wstrb_o == 8'b0)
          $fatal(1, "write transaction has no byte strobe at %h", d_addr_o);
  ...
  task automatic check64(
    ...
    begin
      ...
      if (actual !== expected)
        $fatal(1, "signature %s at +%0d: expected %016h, got %016h",
               name, offset, expected, actual);
      $display("[PASS] %-18s +0x%03x = %016h", name, offset, actual);
    end
  endtask
  ...
  initial begin
    ...
    check64(16'h000, 64'hffff_ffff_ffff_ffeb, "mul");
    check64(16'h008, 64'hffff_ffff_ffff_ffff, "mulh");
    check64(16'h010, 64'hffff_ffff_ffff_fff9, "mulhsu");
    check64(16'h018, 64'hffff_ffff_ffff_fffe, "mulhu");
    check64(16'h020, 64'hffff_ffff_ffff_fffe, "div");
    check64(16'h028, 64'h7fff_ffff_ffff_ffff, "divu");
    check64(16'h030, 64'hffff_ffff_ffff_ffff, "rem");
    check64(16'h038, 64'h0000_0000_0000_000f, "remu");
    check64(16'h040, 64'hffff_ffff_8000_0000, "mulw");
    check64(16'h048, 64'hffff_ffff_ffff_fffe, "divw");
    check64(16'h050, 64'h0000_0000_7fff_ffff, "divuw");
    check64(16'h058, 64'hffff_ffff_ffff_ffff, "remw");
    check64(16'h060, 64'h0000_0000_7fff_ffff, "remuw");
    check64(16'h068, 64'hffff_ffff_ffff_ffff, "div zero");
    check64(16'h070, 64'hffff_ffff_ffff_fff9, "rem zero");
    check64(16'h078, 64'h8000_0000_0000_0000, "div overflow");
    check64(16'h080, 64'h0000_0000_0000_0000, "rem overflow");
    check64(16'h088, 64'hffff_ffff_8000_0000, "divw overflow");
    check64(16'h090, 64'h0000_0000_0000_0000, "remw overflow");
    check64(16'h098, 64'h0000_0000_0000_0007, "m to addi forwarding");
    check64(16'h0a0, 64'h0000_0000_0000_000c, "back-to-back m");

    if (fetch_cycles < 54 || write_cycles != 21 || read_cycles != 0)
      $fatal(1, "unexpected traffic: fetch=%0d read=%0d write=%0d",
             fetch_cycles, read_cycles, write_cycles);
    if (cycle_count <= fetch_cycles + 20)
      $fatal(1, "MDU did not introduce observable EX stalls");

    $display("RV64M smoke test PASS: cycles=%0d fetches=%0d writes=%0d",
             cycle_count, fetch_cycles, write_cycles);
    $finish;
  end
