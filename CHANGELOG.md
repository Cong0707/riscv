...
* 将同步异常改为等待最后一个更老 EX/MEM 结果完成 WB 后再发布，新增较老写回已退休且年轻访存无副作用的精确异常回归。
* 固定当前 D-port 投影为“传输首字节地址、数据从 lane 0 起连续排列、低位连续 strobe”，外部对齐总线由适配器平移 lane。
* 固定 Berkeley HardFloat Release 1 的 RISC-V Verilog 源码、许可证和一行 Icarus 12 兼容补丁，并增加 F32 加法与 F64 迭代除法的双模拟器独立回归；该内核尚未接入核心 F/D 数据通路。
