...
* 项目只维护 `main`，不建立 `develop`、`feature/*` 或其他长期分支。
* 每个提交必须保持可构建、可回归；不完整实验只允许放在 `D:\Develop\AI\codex-work\riscv`。
* 推送前先同步远端并确认没有分叉；已推送历史禁止重写，修复使用新的中文提交。

### 8.2 提交

提交标题和正文使用中文，例如“功能：实现 RV64M 乘除单元”“修复：处理除零边界”“测试：增加 RV64M 回归”。提交正文说明改变了什么、为何改变、如何验证、是否影响 ISA/协议/时序。一个提交尽量对应一个可审查主题，不混入格式化噪声、生成物或无关重构。
